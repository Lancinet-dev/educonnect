  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {classes?.length === 0 && (
          <p className="text-surface-500 text-sm">Aucune classe ne vous est affectée.</p>
        )}
      </div>
    </div>
  )
...
function CallSheet({ classId, onBack }) {
  ...
  // Initialiser/synchroniser les statuts quand les données arrivent
  useEffect(() => {
    if (data?.students) {
      ...
      <Card padding={false} className="divide-y divide-surface-100">
        {data.students.map((s) => (
          <div key={s.id} className="flex items-center gap-3 p-3 sm:p-4">
            ...
            <p className="flex-1 font-medium text-surface-900 text-sm sm:text-base">
        ...
        <div className="fixed bottom-0 left-0 right-0 lg:left-64 bg-white border-t border-surface-200 p-4 z-10">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
            <p className="text-sm text-surface-500 hidden sm:block">
              {Object.keys(statuses).length} élève(s) · {date}
            </p>
...
export default function TeacherAttendance() {
  ...
  if (!classId) {
    return <ClassPicker onPick={(id) => setParams({ classId: id })} />
  }
  return <CallSheet classId={classId} onBack={() => navigate('/teacher/presences')} />
}
